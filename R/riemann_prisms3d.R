...
riemann_prisms3d <- function(
  ...
  as_colorscale <- function(x, alpha=NULL) {
    ...
    if (is.character(x) && length(x)==1) {
      if (is_color(x)) { ccol <- as_rgba(x, alpha); return(list(list(0,ccol), list(1,ccol))) }
  ...
  eval_cell_stats <- function(x0, x1, y0, y1) {
    ...
    ys <- seq(y0, y1, length.out = sample_n + 1)
    Z  <- outer(ys, xs, function(yy,xx) F(xx,yy))
    c(z_lower = min(Z, na.rm=TRUE),
      z_upper = max(Z, na.rm=TRUE),
      z_mean  = mean(Z, na.rm=TRUE))
  }

  for (i in seq_len(N)) {
    x0 <- xs_nodes[i]; x1 <- xs_nodes[i+1]
    ...
    yL <- max(yl0, yl1); yH <- min(yh0, yh1)
    if (yL >= yH) next  # strip has no feasible y at both ends

    for (j in seq_len(M)) {
      yy0 <- y_nodes[j]; yy1 <- y_nodes[j+1]
      # intersect this cell with [yL,yH]
      y0 <- max(yy0, yL); y1 <- min(yy1, yH)
      if (y0 >= y1) next

      zstat <- eval_cell_stats(x0, x1, y0, y1)
      cells[[length(cells)+1L]] <- c(x0=x0, y0=y0, hx=x1-x0, hy=y1-y0, zstat)
    }
  }

  if (!length(cells)) {
    warning("No valid cells for these parameters.")
    return(list(cells=tibble::tibble(), sum_lower=0, sum_upper=0, sum_mean=0, fig=NULL))
  }
    ...
    if (!requireNamespace("plotly", quietly = TRUE)) {
      ...
      add_side <- function(fig, x, y, z0, z1, horiz = c("x","y")) {
        ...
        plotly::add_surface(
          ...
          showscale = FALSE, opacity = side_opacity
        )
      }
      ...
      if (isTRUE(show_surface)) {
        ...
        for (j in seq_len(nx)) Zs[,j] <- vapply(ys, function(yy) F(xs[j],yy), numeric(1))
      ...
      z_col <- switch(color_by, lower=cells$z_lower, upper=cells$z_upper, mean=cells$z_mean)
      for (i in seq_len(nrow(cells))) {
        ...
        fig <- add_side(fig, x=c(x0,x1), y=y0, z0=0, z1=zt, horiz="x") # front (y=y0)
        fig <- add_side(fig, x=c(x0,x1), y=y1, z0=0, z1=zt, horiz="x") # back  (y=y1)
        fig <- add_side(fig, x=x0, y=c(y0,y1), z0=0, z1=zt, horiz="y") # left  (x=x0)
        fig <- add_side(fig, x=x1, y=c(y0,y1), z0=0, z1=zt, horiz="y") # right (x=x1)
  ...
  out <- list(cells = cells,
              sum_lower = sum_lower,
              sum_upper = sum_upper,
              sum_mean  = sum_mean,
              fig = fig)
  if (estimate != "all") {
    key <- switch(estimate, lower="sum_lower", upper="sum_upper", mean="sum_mean")
